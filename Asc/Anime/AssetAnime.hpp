...
namespace asc
{
	...
	public:
		...
		/// 1コマの描画時間[ミリ秒]
		/// </param>
		AssetAnime(const TextureAssetName& name, size_t size, int32 duration, StartImmediately startImmediately = StartImmediately::Yes);
		...
		/// 各コマの描画時間[ミリ秒]
		/// </param>
		AssetAnime(const TextureAssetName& name, size_t size, const Array<int32>& duration, StartImmediately startImmediately = StartImmediately::Yes);
		...
		/// </returns>
		bool preload() { return TextureAsset::Load; };
		...
		/// </summary>
		TextureAsset::IDType id() const { return TextureAsset(m_name).id(); };
